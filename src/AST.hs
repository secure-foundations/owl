...
data Timestamped a = Timestamped {
    _timestampOf :: Ignore Timestamp,
    _tsval :: a
}
    deriving (Generic, Typeable)

instance Show a => Show (Timestamped a) where
    show (Timestamped _ v) = show v
...
mkTimestamped :: Timestamp -> a -> Timestamped a
mkTimestamped ts x = Timestamped (ignore ts) x
...
instance Alpha Timestamp
instance Subst Idx Timestamp
instance Subst AExpr Timestamp
instance Subst ResolvedPath Timestamp

data TimestampConstraint = TimestampLeq Timestamp Timestamp
    deriving (Show, Generic, Typeable, Eq, Ord)

instance Alpha TimestampConstraint
instance Subst Idx TimestampConstraint
instance Subst AExpr TimestampConstraint
instance Subst ResolvedPath TimestampConstraint
...
type Expr = Spanned (Timestamped ExprX)
...
instance Alpha a => Alpha (Timestamped a)

instance Subst b a => Subst b (Timestamped a)
...
-- Lens to access ExprX directly from Expr without needing e^.val.tval
exprVal :: Lens' Expr ExprX
exprVal = val . tsval
