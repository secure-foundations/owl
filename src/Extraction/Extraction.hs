...
extractStruct owlName owlFields = do
    ...
    let typeDef = 
            ...
            owlpretty "pub struct" <+> owlpretty name <+> braces (line <> (
                        vsep . punctuate comma . map (\(n, t) -> owlpretty "pub" <+> owlpretty n <+> owlpretty ":" <+> owlpretty t) $ rustFieldsConcrete'
                    ) <> line)
                ...
                (owlName, (rustifyName owlName, ADT (rustifyName owlName), False, \args -> return $ show $
                        (owlpretty . rustifyName) owlName <+>
                            (braces . hsep . punctuate comma . mapMaybe (\((f,ft), (t,a)) -> (
                                ...
                                (Nothing, _) -> Nothing
                                (Just VecU8, Rc VecU8) -> Just $ f' <+> owlpretty "clone_vec_u8" <> parens (pretty "&*" <> owlpretty a)
