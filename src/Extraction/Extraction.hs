...
extractStruct owlName owlFields = do
    ...
    fieldsUnrustified <- mapM (\(s,t) -> do
        ct <- concretifyTy t
        case ct of CTGhost -> return (s, Nothing) ; t' -> return (s, Just t')) $ owlFields
    rustFields <- mapM (\(s,t) -> do t' <- mapM rustFieldTy t ; return (s, t')) fieldsUnrustified
    let fields = map (\(s,t) -> (rustifyName s, t)) fieldsUnrustified
    let rustFields' = map (\(s,t) -> (rustifyName s, t)) rustFields
    let fieldsConcrete = mapMaybe liftMaybe fields
    let rustFieldsConcrete = mapMaybe liftMaybe rustFields
    let rustFieldsConcrete' = mapMaybe liftMaybe rustFields'
    let typeDef = 
            let needsLifetime t = case t of OwlBuf -> True ; ADT _ -> True ; _ -> False in
            owlpretty "/* TODO this will be generated by parsley */" <> line <>
            owlpretty "pub struct" <+> owlpretty name <> owlpretty "<'a>" <+> braces (line <> (
                        vsep . punctuate comma . map (
                            \(n, t) -> owlpretty "pub" <+> owlpretty n <+> owlpretty ":" <+> owlpretty t <> if needsLifetime t then owlpretty "<'a>" else owlpretty ""
                        ) $ rustFieldsConcrete'
                    ) <> line)
    layout <- layoutStruct UnboundedAllowed name fieldsConcrete
    let isVest = canBeVest layout
    ...
    where
            ...
            M.fromList $
                ...
                (owlName, (rustifyName owlName, ADT (rustifyName owlName), False, \args -> return $ show $
                        (owlpretty . rustifyName) owlName <+>
                            (braces . hsep . punctuate comma . mapMaybe (\((f,ft), (t,a)) -> (
                                let f' = (owlpretty . rustifyName) f <+> owlpretty ":" in
                                case (ft, t) of
                                -- ADT _ -> parens (owlpretty "*" <> owlpretty a <> owlpretty ".")
                                (Nothing, _) -> Nothing
                                (Just VecU8, OwlBuf) -> Just $ f' <+> owlpretty "slice_to_vec" <> parens (owlpretty a <> owlpretty ".as_slice()")
                                (Just VecU8, VecU8) -> Just $ f' <+> owlpretty "clone_vec_u8" <> parens (pretty "&" <> owlpretty a)
                                (Just VecU8, Number) -> Just $ f' <+> owlpretty "owl_counter_as_bytes" <> parens (pretty "&" <> owlpretty a)
                                _ -> Just $ f' <+> owlpretty a))
                            $ zip owlFields args
                        ))) :
                map (\(owlField, _) -> (owlField, (rustifyName owlName, SliceU8, True, \args -> do
                    case args of
                      (ADT owlName, arg) : _ -> do
                        return $ show $
                            owlpretty "vec_as_slice" <> parens (owlpretty "&" <> owlpretty arg <> owlpretty "." <> owlpretty (rustifyName owlField))
                      _ -> throwError $ TypeError $ "attempt to get from " ++ owlName ++ " with bad args"
                ))) (catMaybes . map liftMaybe $ owlFields)
        ...
        genParsleyWrappers :: String -> [(String, RustTy)] -> Bool -> ExtractionMonad OwlDoc
        genParsleyWrappers owlName rustFields' isVest = do
            let name = rustifyName owlName 
            let specname = specName owlName 
            ...
            --         owlpretty "        res is Some ==> res.get_Some_0()." <> owlpretty (rustifyName n) <> owlpretty ".dview() == " <> 
            ...
            let parserBody = 
                    ...
                    owlpretty "} else { None }" 
            let fieldLen f = owlpretty "arg." <> owlpretty f <> owlpretty ".len()"
            ...
            return $
                ...
                lbrace <> line <>
                    (if isVest then 
                        ...
                        owlpretty "assume(res is Some); res.unwrap()" <> line
                    else owlpretty "// can't autogenerate vest serializer" <> line <> owlpretty "todo!()") <> line <>
                rbrace <> line <> line 
...
extractEnum owlName owlCases' = do
    ...
    cases <- M.fromList <$> (forM owlCases' $ \(s, ot) -> do
        let rs = rustifyName s
        oct <- traverse concretifyTy ot
        return (rs, oct))
    rustCases <- mapM (\o -> case o of Just t -> (do t' <- concretifyTy t >>= rustFieldTy; return (Just t')); Nothing -> return Nothing) owlCases
    let rustCases' = M.mapKeys rustifyName rustCases
    let typeDef = 
            owlpretty "/* TODO this will be generated by parsley */" <> line <>
            {- owlpretty "#[is_variant]" <> line <> -}
            owlpretty "pub enum" <+> owlpretty name <> owlpretty "<'a>" <+> braces (line <> (
                        vsep . punctuate comma . map (
                            \(n, t) -> owlpretty n <> parens (owlpretty t <> if t == Just OwlBuf then owlpretty "<'a>" else owlpretty "")
                        ) $ M.assocs rustCases'
    ...
    where
        ...
        genParsleyWrappers owlName rustCases' = 
            ...
            let parseEnsuresLenValid (c,topt) =
                    case topt of
                        Nothing -> owlpretty ""
                        Just OwlBuf -> owlpretty "res matches Some(" <> owlpretty c <> owlpretty "(x)) ==> x.len_valid(),"
                        Just _ -> owlpretty "" 
                    in
            let serRequiresLenValid (c,topt) =
                    case topt of
                        Nothing -> owlpretty ""
                        Just OwlBuf -> owlpretty "arg matches " <> owlpretty c <> owlpretty "(x) ==> x.len_valid(),"
                        Just _ -> owlpretty "" 
                    in
            ...
            --                                      specParse <> owlpretty ".get_Some_0()." <> owlpretty (specName n) <> owlpretty ","
            -- exec parser
            owlpretty "/* TODO should be provable once vest integrated */ #[verifier(external_body)]" <> line <>
    ...
    (rt, preCryptOp, str) <- case (op, args) of
        (CKDF _ _ nks nkidx, [salt, ikm, info]) -> do
            ...
            (genOrcl, orclName) <- case oopt of
                Just (OwlBuf, name) -> return (pretty "", name)
                Nothing -> do
                    rovar' <- fresh . s2n $ "kdfval"
                    let rovar = rustifyName . show $ rovar'
                    kdfCalls %= (:) ((nks, resolvedArgs), (OwlBuf, rovar))
                    let kdfArgs = [(Number, outLen), salt, ikm, info]
                    let genOrcl = 
                            owlpretty "let" <+> owlpretty rovar <+> owlpretty "=" <+>
                            owlpretty "OwlBuf::from_vec" <> parens (owlpretty (printOwlOp "owl_extract_expand_to_len" kdfArgs)) <> owlpretty ";"
                    return (genOrcl, rovar)
                _ -> throwError $ ErrSomethingFailed "precomputed hash value has wrong type"
            let sliceOrcl = owlpretty orclName <> owlpretty ".subrange" <> parens (owlpretty start <> comma <+> owlpretty end)
                                ...
                                --     )
                                -- )
            return (OwlBuf, genOrcl, sliceOrcl)
        ...
        (CEncStAEAD np _ xpat, [k, x, aad]) -> do -- TODO handle pattern
            ...
            return (VecU8, owlpretty "", unwrapped)
        (CDecStAEAD, [k, c, aad, (nty, narg)]) -> do 
            let n = case nty of
                    Number -> (VecU8, printOwlOp "owl_counter_as_bytes" [(nty, narg)])
                    _ -> (nty, narg)
            return (Option $ VecU8, owlpretty "", owlpretty $ printOwlOp "owl_dec_st_aead" [k, c, n, aad])
        (CPKEnc, [k, x]) -> do return (VecU8, owlpretty "", owlpretty $ printOwlOp "owl_pkenc" [k, x])
        (CPKDec, [k, x]) -> do return (VecU8, owlpretty "", owlpretty $ printOwlOp "owl_pkdec" [k, x])
        (CMac, [k, x]) -> do return (VecU8, owlpretty "", owlpretty $ printOwlOp "owl_mac" [k, x])
...
extractAExpr binds (AEGet nameExp) = do
    ...
    return (SliceU8, owlpretty "", owlpretty "vec_as_slice" <> parens (owlpretty "&self." <> owlpretty fnameExp))
...
extractExpr inK loc binds (CInput xsk) = do
    ...
    (_, rt', prek, kPrettied) <- extractExpr inK loc (M.insert rustX (OwlBuf, Nothing) binds) k
    let eWrapped = owlpretty "OwlBuf::from_vec" <> parens (owlpretty "temp_" <> owlpretty rustX)
    typeAnnot <- do
        t <- getCurRetTy
        return $ owlpretty "::" <> angles (owlpretty t)
    let letbinding =
                ...
                case o of
                    ...
                    Right xk -> do
                        ...
                        let wrappedRt = case rt' of
                                ...
                                SliceU8 -> OwlBuf
                                _ -> rt'
                        (_, rt'', preK, kPrettied) <- extractExpr inK loc (M.insert rustX (wrappedRt, Nothing) binds) k
                        let eWrapped = case rt' of
                                VecU8 -> owlpretty "OwlBuf::from_vec" <> parens (owlpretty "temp_" <> owlpretty rustX)
                                SliceU8 -> owlpretty "OwlBuf::from_slice" <> parens (owlpretty "temp_" <> owlpretty rustX)
                                _ -> owlpretty "temp_" <> owlpretty rustX
                ...
                case o of
                    ...
                    Right xk -> do
                        ...
                        (caseRustTy, caseName, makeCaseName) <- case enumCases M.!? c of
                                Just (Just VecU8) -> do 
                                    let makeCaseName = owlpretty "let" <+> owlpretty rustX <+> owlpretty "=" <+> owlpretty "OwlBuf::from_vec" <> parens (owlpretty "tmp_" <> owlpretty rustX) <> owlpretty ";"
                                    return (OwlBuf, "tmp_" ++ rustX, makeCaseName)
...
extractExpr inK loc binds (CTLookup tbl ae) = do
    ...
    aeWrapped <- case rt of
            OwlBuf -> return $ parens aePrettied <> owlpretty ".as_slice()"
...
extractExpr inK loc binds (CParse ae owlT@(CTConst p) badkopt bindpat) = do 
    ...
    let destructStruct (v, (f, t)) = 
            let (wrapFn, closeparen) = case t of
                    VecU8 -> (owlpretty "OwlBuf::from_vec(", owlpretty ")")
                    SliceU8 -> (owlpretty "OwlBuf::from_slice(", owlpretty ")")
                    _ -> (owlpretty "", owlpretty "") in
            owlpretty "let" <+> owlpretty (rustifyName . show $ v) <+> owlpretty "=" <+> 
            wrapFn <> owlpretty "parseval." <> owlpretty (rustifyName f) <> closeparen <> owlpretty ";"
    case (rtAe, badkopt) of
        (ADT r, _) | r == rustifyName t -> do
            ...
            -- let destructStruct (v, (f, t)) = owlpretty "let" <+> owlpretty (rustifyName . show $ v) <+> 
...
rustifyArg (v, t) = do
    rt <- (concretifyTy $ unembed t) >>= rustifyArgTy 
...
makeFunc owlName _ owlArgs owlRetTy = do
    ...
    rtb <- concretifyTy owlRetTy >>= rustifyRetTy
    ...
    where
        ...
        cfgFields idxs localNames sharedNames pubKeys tbls =
            vsep . punctuate comma $
                ...
                map (\(s,_,npids) -> owlpretty "pub" <+> owlpretty (rustifyName s) <> owlpretty ": Vec<u8>") localNames ++
                map (\(s,_,npids) -> owlpretty "pub" <+> owlpretty (rustifyName s) <> (if npids == 0 || (idxs == 1 && npids == 1) then owlpretty ": Vec<u8>" else owlpretty ": HashMap<usize, Vec<u8>>")) sharedNames ++
                map (\(s,_,_) -> owlpretty "pub" <+> owlpretty "pk_" <> owlpretty (rustifyName s) <> owlpretty ": Vec<u8>") pubKeys ++
            ...
            return $ owlpretty "#[verifier(external_body)] pub fn init_" <> owlpretty (cfgName loc) <+> parens (owlpretty "config_path : &StrSlice") <+> owlpretty "-> Self" <+> lbrace <> line <>
                ...
                owlpretty "return" <+> owlpretty (cfgName loc) <+>
                    braces (hsep . punctuate comma $
                        owlpretty "listener"  :
                        map (\(s,_,_) ->
                                (owlpretty . rustifyName $ s) <+> owlpretty ":" <+> parens (owlpretty . rustifyName $ s)
                            ) localNames ++
                        map (\(s,_,_) -> owlpretty (rustifyName s) <+> owlpretty ":" <+> parens (owlpretty "config." <> owlpretty (rustifyName s))) sharedNames ++
                        map (\(s,_,_) -> owlpretty "pk_" <> owlpretty (rustifyName s) <+> owlpretty ":" <+> parens (owlpretty "config." <> owlpretty "pk_" <> owlpretty (rustifyName s))) pubKeys ++
                        map (\(n,_) -> owlpretty (rustifyName n) <+> owlpretty ":" <+> owlpretty "HashMap::new()") tbls ++
                        [owlpretty "salt :" <+> owlpretty "(config.salt)"]
                    ) <>
                rbrace
...
extractTyDefs ((tv, td):ds) = do
    ...
    where
        ...
        extractTyDef name (TB.TyAbbrev t) = do
            lct <- concretifyTy t >>= layoutCTy UnboundedAllowed 
