...
extractStruct owlName owlFields = do
    ...
    let typeDef = 
            ...
            owlpretty "pub struct" <+> owlpretty name <> owlpretty "<'a>" <+> braces (line <> (
                        vsep . punctuate comma . map (
                            \(n, t) -> owlpretty "pub" <+> owlpretty n <+> owlpretty ":" <+> owlpretty t <> if t == OwlBuf then owlpretty "<'a>" else owlpretty ""
                        ) $ rustFieldsConcrete'
                    ) <> line)
    ...
    parsleyWrappers <- genParsleyWrappers owlName rustFieldsConcrete'
