            ...
            case nd of
              ...
              TB.BaseDef (nt, loc) -> do
                ...
                case nt ^.val of
                    ...
                    NT_PKE _ ->
                        return (foldl gShared locMap locNames, shared ++ [((name, flen, npids, BufSecret), locNames)], pubkeys ++ [(name, flen, npids, BufPublic)])
                    NT_Sig _ ->
                        return (foldl gShared locMap locNames, shared ++ [((name, flen, npids, BufSecret), locNames)], pubkeys ++ [(name, flen, npids, BufPublic)])
                    NT_DH ->
                        return (foldl gShared locMap locNames, shared ++ [((name, flen, npids, BufSecret), locNames)], pubkeys ++ [(name, flen, npids, BufPublic)])
                    _ -> if length loc /= 1 then
                            ...
                            return (foldl gShared locMap locNames, shared ++ [((name, flen, npids, BufSecret), locNames)], pubkeys)
                        else
                            -- name is local and can be locally generated
                            return (foldl gLocal locMap locNames, shared, pubkeys)
