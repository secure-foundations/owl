...
pub mod secret {
    ...
    impl<'x> SecretBuf<'x> {
        ...
        pub fn private_as_slice(&self) -> (result: &[u8])
            ensures  result.view() == self.view()
        {
            reveal(SecretBuf::len_valid);
            self.buf.as_slice()
    ...
    impl<'a> Builder for OwlStAEADBuilder<'a> {
        ...
        fn into_mut_vec(&self, data: &mut Vec<u8>, pos: usize) {
            ...
            let iv = self.iv.private_as_slice();
            ...
            view_option(x) == dec_st_aead(k.view(), c.view(), nonce.view(), aad.view())
    {
        ...
        let mut iv_sized = [0u8; 12];
        let iv_len = iv_sized.len();
        let nonce = nonce.private_as_slice();
        iv_sized[(iv_len - nonce.len())..].copy_from_slice(&nonce[..]);
        match owl_aead::decrypt_combined(CIPHER, k.private_as_slice(), c.as_slice(), &iv_sized[..], aad.private_as_slice()) {
            Ok(p) => Some(OwlBuf::from_vec(p).into_secret()),
            Err(_e) => {
                // dbg!(e);
                None
...
pub use crate::wireguard::owl_wg::execlib::secret::*;
