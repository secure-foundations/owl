	...
	for elemsContainer := range device.queue.encryption.c {
		for _, elem := range elemsContainer.elems {	
			...
			C.wg_send(
				...
				C.ulong(MessageTransportHeaderSize + plaintext_len + chacha20poly1305.Overhead));
			

			C.free(unsafe.Pointer(plaintext_str));
			C.free(unsafe.Pointer(key_str));
			
			elem.packet = elem.buffer[:(MessageTransportHeaderSize + plaintext_len + chacha20poly1305.Overhead)]
